...
router.put("/companies/:company_id/exercises/:exercise_id", asyncHandler((req, res) => exerciseControllers.updateExercise(req, res)));
router.delete("/companies/:company_id/exercises/:exercise_id", asyncHandler((req, res) => exerciseControllers.deleteExercise(req, res)));
//Planos
router.post("/therapists/:therapists_id/plans", asyncHandler((req, res) => planControllers.createPlan(req, res)));
router.get("/patients/:patients_id/plans", asyncHandler((req, res) => planControllers.getPlansByPatientId(req, res)));
router.post("/patients/:patients_id/plans/:plans_id/exercises/:exercises_id/day/:day/done", asyncHandler((req, res) => 
//"Paciente marcar um dia do exercício de um plano como feito.
planControllers.onPlanMarkDayAsDone(req, res)));
router.get("/patients/:patients_id/plans/:plans_id", asyncHandler((req, res) => 
//res.send("Obter informação de um plano especifico de um paciente.")
planControllers.getPlanByIdAndPatientId(req, res)));
router.get("/therapists/:therapists_id/patients/:patients_id/plans/:plans_id/progress", asyncHandler((req, res) => 
