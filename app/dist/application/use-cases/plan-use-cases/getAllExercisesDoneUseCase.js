...
export default function getAllExercisesDone(Repository) {
    return async (id_pac) => {
        let exercisesDone = [];
        const data = await Repository.getPlansByPatientId(id_pac);
        if (!data)
            throw ErrorTypes.NotFoundError("No exercises found");
        if (data.length === 0)
            throw ErrorTypes.NotFoundError("No exercises found");
        for (let i = 0; i < data.length; i++) {
            if (!data[i].plan_exercises)
                throw ErrorTypes.NotFoundError("No exercises found");
            if (data[i].plan_exercises.length === 0)
                throw ErrorTypes.NotFoundError("No exercises found");
            for (let j = 0; j < data[i].plan_exercises.length; j++) {
                if (!data[i].plan_exercises[j].exercise)
                    throw ErrorTypes.NotFoundError("No exercises found");
